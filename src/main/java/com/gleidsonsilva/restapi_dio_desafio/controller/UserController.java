...
import com.gleidsonsilva.restapi_dio_desafio.controller.dto.UserDto;
...
public record UserController (UserService userService) {
    ...
    public ResponseEntity<UserDto> update(@PathVariable Long id, @RequestBody UserDto userDto) {
        var user = userService.update(userDto.toModel(), id);
        return ResponseEntity.ok(new UserDto(user));
    }

    @DeleteMapping("/{id}")
    @Operation(summary = "Delete a user", description = "Delete an existing user bases on its ID")
    @ApiResponses(value = {
